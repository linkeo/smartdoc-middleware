  ...
  function readDoc(comment, node, moduleStack) {
    if (comment && comment.type === 'Block' && comment.value) {
      ...
      if (isDocumentComment) {
        if (comment.value.indexOf('@application') !== -1) { return readApplicationDoc(content); }
        if (comment.value.indexOf('@module') !== -1) { return readModuleDoc(content); }
        if (comment.value.indexOf('@route') !== -1) { return readActionDoc(content); }
      }
    }
  }
